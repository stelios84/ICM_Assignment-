...
namespace Domain.Aggregates.OldAR
{
    public class BTCTest3
    {
        ...
        public BTCTest3( string name, int height, string hash, DateTime time, string latestUrl, string previousHash, string previousUrl, int? peerCount, int? unconfirmedCount, int? highFeePerKb, int? mediumFeePerKb, int? lowFeePerKb, int? lastForkHeight, string lastForkHash, DateTime createdAt,string jsonApi)
        {
            _name = name;
            _height = height;
            _hash = hash;
